...
RSpec.feature "Contact user", :type => :feature do
  ...
  let(:category) {create(:category, name: 'Arts', brach: 'hobby')}
  ...
  context 'logged in user' do
    ...
    scenatio "successfully sends a message to a post's author", js: true do
      ...
      expect(page).to have_selector('.contact-user', text: 'Message has been sent')
    end

    scenario 'sees an already contacted message' do
      create(:privatye_conversation_with_messages,
      ...
      expect(page).to have_selector(
        ...
        text: 'You area already in touch with this user'
      )
    end
  end

  context 'non-logged in user' do
    scenatio 'sees a login required message to contact a user' do
      visit post_path(post)
